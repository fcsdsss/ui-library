...
do
	function library:Window(windowInfo)
        ...
        local toggleButton = utility:Create({Type = "TextButton", Properties = {Parent = screen, Size = UDim2.new(0, 80, 0, 30), Position = UDim2.new(0, 10, 0, 10), BackgroundColor3 = Color3.fromRGB(30, 30, 30), BorderSizePixel = 0, Text = "Toggle UI", TextColor3 = Color3.fromRGB(255, 255, 255), Font = "Code", TextSize = 14, ZIndex = 9999}})
		...
		local title = utility:Create({Type = "TextLabel", Properties = {BackgroundTransparency = 1, BorderSizePixel = 0, Parent = frame, Position = UDim2.new(0, 9, 0, 6), Size = UDim2.new(1, -16, 0, 15), Font = "Code", RichText = true, Text = info.Name or "obleus", TextColor3 = Color3.fromRGB(142, 142, 142), TextSize = 13, TextXAlignment = "Left"}})
		...
		-- [修复] 替换强调色
		utility:Create({Type = "Frame", Properties = {BackgroundColor3 = Color3.fromRGB(200, 200, 200), BorderSizePixel = 0, Parent = accent, Position = UDim2.new(0, 0, 0, 0), Size = UDim2.new(1, 0, 0, 1)}})
		utility:Create({Type = "Frame", Properties = {BackgroundColor3 = Color3.fromRGB(150, 150, 150), BorderSizePixel = 0, Parent = accent, Position = UDim2.new(0, 0, 0, 1), Size = UDim2.new(1, 0, 0, 1)}})
		...
		function window:Page(pageInfo)
			...
			function page:Turn(state)
				-- [修复] 替换激活标签页的文字颜色
				page.Open = state; tabTitle.TextColor3 = state and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(142, 142, 142); tabGradient.Color = state and ColorSequence.new({ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 255, 255)), ColorSequenceKeypoint.new(1, Color3.fromRGB(155, 155, 155))}) or ColorSequence.new({ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 255, 255)), ColorSequenceKeypoint.new(1, Color3.fromRGB(100, 100, 100))}); pageHolder.Visible = state;
			end
			...
			function page:Section(sectionInfo)
				...
				function section:Dropdown(dropdownInfo)
					...
					local h = utility:Create({Type = "Frame", Properties = {BackgroundTransparency = 1, Parent = sectionContentHolder, Size = UDim2.new(1, 0, 0, 50), LayoutOrder = #sectionContentHolder:GetChildren() + 1}})
					if info.Name then utility:Create({Type = "TextLabel", Properties = {Name = "Label", Text = info.Name, Font = "Code", TextColor3 = Color3.fromRGB(180, 180, 180), TextSize = 13, TextXAlignment = "Left", BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 16), Parent = h}}) end
					
					local selectionButton = utility:Create({Type = "TextButton", Properties = {Name = "Selection", Font = "Code", TextColor3 = Color3.fromRGB(180, 180, 180), TextSize = 14, TextXAlignment = "Left", BackgroundColor3 = Color3.fromRGB(23, 23, 23), Size = UDim2.new(1, 0, 0, 30), Position = UDim2.new(0, 0, 0, 20), Text = "  " .. selectedOption, Parent = h, ZIndex = 2}})
					utility:Create({Type = "UICorner", Properties = {CornerRadius = UDim.new(0, 3), Parent = selectionButton}})
				
					local arrowIcon = utility:Create({Type = "ImageLabel", Properties = {Name = "Arrow", Image = "rbxassetid://6031091004", BackgroundTransparency = 1, AnchorPoint = Vector2.new(1, 0.5), Position = UDim2.new(1, -8, 0.5, 0), Size = UDim2.new(0, 16, 0, 16), Rotation = 0, Parent = selectionButton}})
					
					local optionsFrame = utility:Create({Type = "Frame", Properties = {Name = "OptionsFrame", BackgroundColor3 = Color3.fromRGB(25, 25, 25), BorderSizePixel = 0, Size = UDim2.new(0, 0, 0, 0), Visible = false, ZIndex = 100, ClipsDescendants = true, Parent = screen}})
					utility:Create({Type = "UICorner", Properties = {CornerRadius = UDim.new(0, 3), Parent = optionsFrame}})
				
					local optionsList = utility:Create({Type = "ScrollingFrame", Properties = {Name = "OptionsList", BackgroundTransparency = 1, BorderSizePixel = 0, Size = UDim2.new(1, 0, 1, 0), AutomaticCanvasSize = "Y", ScrollBarThickness = 4, ScrollBarImageColor3 = Color3.fromRGB(220, 220, 220), ZIndex = 101, Parent = optionsFrame}})
					...
					local function closeDropdown()
						...
						local tween = ts:Create(optionsFrame, TweenInfo.new(0.2), {Size = UDim2.new(optionsFrame.AbsoluteSize.X, 0, 0, 0)})
					...
					local function openDropdown()
						...
						ts:Create(optionsFrame, TweenInfo.new(0.2), {Size = UDim2.new(0, absSize.X, 0, targetHeight)}):Play()
					...
					for _, optionName in ipairs(options) do
						local optionButton = utility:Create({Type = "TextButton", Properties = {Name = optionName, Text = "  " .. optionName, Font = "Code", TextColor3 = Color3.fromRGB(180, 180, 180), TextSize = 14, TextXAlignment = "Left", BackgroundColor3 = Color3.fromRGB(45, 45, 45), Size = UDim2.new(1, -4, 0, 24), ZIndex = 102, Parent = optionsList}})
						utility:Create({Type = "UICorner", Properties = {CornerRadius = UDim.new(0, 3), Parent = optionButton}})
						
						optionButton.MouseEnter:Connect(function() ts:Create(optionButton, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(60, 60, 60)}):Play() end)
						optionButton.MouseLeave:Connect(function() ts:Create(optionButton, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(45, 45, 45)}):Play() end)
					...
					function dropdown:Set(newValue) if table.find(options, newValue) then dropdown.state = newValue; selectionButton.Text = "  " .. newValue; callback(newValue) end end
...
local function processNotificationQueue()
	...
	local title, text, duration, color = info.Title or "Notification", info.Text or "", info.Duration or 5, info.Color or Color3.fromRGB(220, 220, 220)
